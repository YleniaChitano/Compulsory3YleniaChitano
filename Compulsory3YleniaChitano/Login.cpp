...
void insert()
{
    std::fstream password("PasswordCheck.txt", std::ios::in | std::ios::out | std::ios::trunc); // Trunc deletes the previous file content
...
void end(bool& loop, std::fstream& password) {
    password.close();
    std::cout << "Password inserted correctly.\n\n";
    loop = false;
}
